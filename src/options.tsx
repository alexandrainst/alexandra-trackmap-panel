  ...
  return (
    builder
      ...
      .addBooleanSwitch({
        ...
        defaultValue: false,
        showIf: config => !config.map.useCenterFromFirstPos && !config.map.useCenterFromLastPos && !config.map.useBoundsInQuery,
      })
      .addNumberInput({
        path: 'map.centerLatitude',
        name: 'Map center latitude',
        defaultValue: 56.17203,
        showIf: config => !config.map.useCenterFromFirstPos && !config.map.useCenterFromLastPos && !config.map.zoomToDataBounds,
      })
      .addNumberInput({
